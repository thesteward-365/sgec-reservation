...
export function HistoryListItem({ item, onClick, showPlaceName }: Props) {
  ...
  const dateChanged =
    ...
    changes?.startTime &&
    !isSameDay(changes.startTime.from, changes.startTime.to);
  const startTimeActuallyChanged =
    item.actionType === 'updated' &&
    changes?.startTime &&
    formatTime(changes.startTime.from) !== formatTime(changes.startTime.to);
  const endTimeActuallyChanged =
    item.actionType === 'updated' &&
    changes?.endTime &&
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        'rounded-lg bg-white p-4 shadow-(--shadow-1) transition-colors',
        ...
        Object.keys(changes).length > 0 ? (
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <div className="flex flex-1 items-center gap-2 overflow-hidden text-[13px]">
                  <span className="truncate line-through opacity-40">
                    {formatDate(changes.startTime.from)}
                  </span>
                  <span className="opacity-30">→</span>
                  <span className="truncate font-semibold text-blue-600">
                    {formatDate(changes.startTime.to)}
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-3 text-sm">
              ...
              <span className="truncate font-semibold">{displayPlaceName}</span>
            </div>
            <div className="flex items-center gap-3 text-sm">
              <span className="text-muted-foreground w-16 shrink-0 font-medium">
                날짜
              ...
              <span className="font-semibold">
                {snapshot?.startTime ? formatDate(snapshot.startTime) : '-'}
              </span>
            </div>
            <div className="flex items-center gap-3 text-sm">
              <span className="text-muted-foreground w-16 shrink-0 font-medium">
              ...
              <span className="font-semibold">
                {snapshot?.startTime ? formatTime(snapshot.startTime) : '-'} ~{' '}
                {snapshot?.endTime ? formatTime(snapshot.endTime) : '-'}
              </span>
            </div>
            <div className="flex items-center gap-3 text-sm">
              <span className="text-muted-foreground w-16 shrink-0 font-medium">
            ...
            {item.actionType === 'cancelled' && (
              <p className="text-caption mt-2 font-bold text-red-500">
                이 예약은 취소되었습니다.
              </p>
            )}
          </div>
