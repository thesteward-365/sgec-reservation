...
- [ ] 상단 헤더: 교회 로고 + "샘깊은교회 / 문화사역 장소방" + 설정 아이콘
- [ ] 인사 문구: "XXX님, 언제 예약하시겠어요?" (세션 사용자 이름 바인딩)
- [ ] "장소" 섹션 헤더 + 우측 "필터" 버튼 추가
- [ ] 층 필터: 인라인 칩 → 바텀시트 (층 + 태그 + 초기화/적용하기)
- [ ] 장소 카드 우측: 꺾쇠(`>`) → 해당 날짜 예약 건수 ("예약 N건", 파란 텍스트)
- [ ] 태그 스타일: outline 배지 → chip-tag (`#태그명`, 회색 배경)
